...
class ROA(object):
    ...
    def gradient_based_search(self, X, y, alpha, num_iter, width, height, xskip, yskip, potential_nums,random = False):
        ...
        output_j = (torch.zeros(y.shape[0]) + output_j1[:,0].float()).to(device)
        output_i = (torch.zeros(y.shape[0]) + output_i1[:,0].float()).to(device)
        ...
        with torch.set_grad_enabled(False):
            for l in range(output_j1.size(1)):
                ...
                padding_j = (torch.zeros(y.shape[0]) + output_j1[:,l].float()).to(device)
                padding_i = (torch.zeros(y.shape[0]) + output_i1[:,l].float()).to(device)
                output_j[all_loss > max_loss] = padding_j[all_loss > max_loss]
                output_i[all_loss > max_loss] = padding_i[all_loss > max_loss]
                max_loss = torch.max(max_loss, all_loss)
